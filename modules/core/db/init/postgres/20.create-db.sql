...
alter table INTERSHIP_CUSTOMER add constraint FK_INTERSHIP_CUSTOMER_ON_USER foreign key (USER_ID) references SEC_USER(ID)^
create unique index IDX_INTERSHIP_CUSTOMER_UK_EMAIL on INTERSHIP_CUSTOMER (EMAIL) where DELETE_TS is null ^
create index IDX_INTERSHIP_CUSTOMER_ON_USER on INTERSHIP_CUSTOMER (USER_ID)^
